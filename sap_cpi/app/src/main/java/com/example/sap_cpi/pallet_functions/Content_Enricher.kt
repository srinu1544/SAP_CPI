...
Flow direction in Content Enricher:
The main message comes from the sender system.
CPI calls the external system (receiver system) to get additional data — this is the enrichment part.
The response from the receiver system is then merged into the main message and sent forward.
->only from receiver system can connect to content enricher
...
LOOKUP MESSAGE :
 ...
 -> when ever lookup message place in between of original message we can use enrich
